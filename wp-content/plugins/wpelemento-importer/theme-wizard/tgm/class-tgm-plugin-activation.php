...
if ( ! class_exists( 'WPELEMENTO_IMPORTER_TGM_Plugin_Activation' ) ) {
	 ...
	 * @author  Gary Jones
	 */
	class WPELEMENTO_IMPORTER_TGM_Plugin_Activation {
		/**
		 ...
		 * @since 1.0.0
		 *
		 * @var WPELEMENTO_IMPORTER_TGM_Plugin_Activation
		 */
		 ...
		 * @var string
		 */
		public $id = 'wpelemento_importer_tgmpa';
		 ...
		 * @var string
		 */
		protected $menu = 'wpelemento-importer-tgmpa-install-plugins';
		 ...
		 * @since 1.0.0
		 *
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::init()
		 */
		 ...
		 * @since 2.0.0
		 *
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::admin_menu()
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::notices()
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::styles()
		 */
		public function init() {
			 ...
			 */
			if ( true !== apply_filters( 'wpelemento_importer_tgmpa_load', ( is_admin() && ! defined( 'DOING_AJAX' ) ) ) ) {
				return;
			}
		...
		public function thickbox() {
			if ( ! get_user_meta( get_current_user_id(), 'wpelemento_importer_tgmpa_dismissed_notice_' . $this->id, true ) ) {
				add_thickbox();
			}
		}
		 ...
		 *
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::init()
		 * @see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::install_plugins_page()
		 *
		...
		public function dismiss() {
			if ( isset( $_GET['tgmpa-dismiss'] ) && check_admin_referer( 'tgmpa-dismiss-' . get_current_user_id() ) ) {
				update_user_meta( get_current_user_id(), 'wpelemento_importer_tgmpa_dismissed_notice_' . $this->id, 1 );
			}
		}
		...
		public function register( $plugin ) {
			...
			$plugin['required']           = WPELEMENTO_IMPORTER_TGMPA_Utils::wpelemento_importer_validate_bool( $plugin['required'] );
			$plugin['force_activation']   = WPELEMENTO_IMPORTER_TGMPA_Utils::wpelemento_importer_validate_bool( $plugin['force_activation'] );
			$plugin['force_deactivation'] = WPELEMENTO_IMPORTER_TGMPA_Utils::wpelemento_importer_validate_bool( $plugin['force_deactivation'] );
		 ...
		 * http://example.com/wp-admin/themes.php?page=wpelemento-importer-tgmpa-install-plugins
		 *
		 * @since 2.5.0
		 *
		 ...
		 *
		 * I.e. depending on the config settings passed something along the lines of:
		 * http://example.com/wp-admin/themes.php?page=wpelemento-importer-tgmpa-install-plugins&plugin_status=install
		 *
		 * @since 2.5.0
		 *
		 ...
		 * @return string Properly encoded URL (not escaped).
		 */
		public function get_wpelemento_importer_tgmpa_status_url( $status ) {
		...
		public function update_dismiss() {
			delete_metadata( 'user', null, 'wpelemento_importer_tgmpa_dismissed_notice_' . $this->id, null, true );
		}
		 ...
		 * @since 2.5.0
		 */
		public function show_wpelemento_importer_tgmpa_version() {
		 ...
		 * @since 2.4.0
		 *
		 * @return \WPELEMENTO_IMPORTER_TGM_Plugin_Activation The WPELEMENTO_IMPORTER_TGM_Plugin_Activation object.
		 */
	...
	if ( ! function_exists( 'load_wpelemento_importer_tgm_plugin_activation' ) ) {
		/**
		 * Ensure only one instance of the class is ever invoked.
		 *
		 * @since 2.5.0
		 */
		function load_wpelemento_importer_tgm_plugin_activation() {
			$GLOBALS['wpelemento_importer_tgmpa'] = WPELEMENTO_IMPORTER_TGM_Plugin_Activation::get_instance();
		}
	}

	if ( did_action( 'plugins_loaded' ) ) {
		load_wpelemento_importer_tgm_plugin_activation();
	} else {
		add_action( 'plugins_loaded', 'load_wpelemento_importer_tgm_plugin_activation' );
	}
}

if ( ! function_exists( 'wpelemento_importer_tgmpa' ) ) {
	/**
...
if ( ! class_exists( 'WPELEMENTO_IMPORTER_TGMPA_List_Table' ) ) {
	 ...
	 * @author  Gary Jones
	 */
	class WPELEMENTO_IMPORTER_TGMPA_List_Table extends WP_List_Table {
		/**
		 * TGMPA instance.
		 *
		 * @since 2.5.0
		...
		protected function _gather_plugin_data() {
			...
			$this->wpelemento_importer->admin_init();
			$this->wpelemento_importer->thickbox();
			...
			foreach ( $plugins[ $this->view_context ] as $slug => $plugin ) {
				...
				$table_data[ $i ]['plugin']            = '<strong>' . $this->wpelemento_importer->get_info_link( $slug ) . '</strong>';
				$table_data[ $i ]['source']            = $this->get_plugin_source_type_text( $plugin['source_type'] );
				$table_data[ $i ]['type']              = $this->get_plugin_advise_type_text( $plugin['required'] );
				$table_data[ $i ]['status']            = $this->get_plugin_status_text( $slug );
				$table_data[ $i ]['installed_version'] = $this->wpelemento_importer->get_installed_version( $slug );
				$table_data[ $i ]['minimum_version']   = $plugin['version'];
				$table_data[ $i ]['available_version'] = $this->wpelemento_importer->does_plugin_have_update( $slug );

				// Prep the upgrade notice info.
				$upgrade_notice = $this->wpelemento_importer->get_upgrade_notice( $slug );
		...
		protected function categorize_plugins_to_views() {
			...
			foreach ( $this->wpelemento_importer->plugins as $slug => $plugin ) {
				if ( $this->wpelemento_importer->is_plugin_active( $slug ) && false === $this->wpelemento_importer->does_plugin_have_update( $slug ) ) {
					...
					if ( ! $this->wpelemento_importer->is_plugin_installed( $slug ) ) {
						...
						if ( false !== $this->wpelemento_importer->does_plugin_have_update( $slug ) ) {
							$plugins['update'][ $slug ] = $plugin;
						}

						if ( $this->wpelemento_importer->can_plugin_activate( $slug ) ) {
			...
			foreach ( $this->view_totals as $type => $count ) {
				...
				if ( ! empty( $text ) ) {
					...
					$status_links[ $type ] = sprintf(
						'<a href="%s"%s>%s</a>',
						esc_url( $this->wpelemento_importer->get_wpelemento_importer_tgmpa_status_url( $type ) ),
		...
		public function extra_tablenav( $which ) {
			if ( 'bottom' === $which ) {
				$this->wpelemento_importer->show_wpelemento_importer_tgmpa_version();
			}
		}
		...
		public function process_bulk_actions() {
			...
			if ( 'wpelemento-importer-tgmpa-bulk-install' === $this->current_action() || 'wpelemento-importer-tgmpa-bulk-update' === $this->current_action() ) {

				check_admin_referer( 'bulk-' . $this->_args['plural'] );

				$install_type = 'install';
				if ( 'wpelemento-importer-tgmpa-bulk-update' === $this->current_action() ) {
					$install_type = 'update';
				}
				...
				$plugins_to_install = array_map( array( $this->wpelemento_importer, 'sanitize_key' ), $plugins_to_install );

				// Validate the received input.
				foreach ( $plugins_to_install as $key => $slug ) {
					// Check if the plugin was registered with TGMPA and remove if not.
					if ( ! isset( $this->wpelemento_importer->plugins[ $slug ] ) ) {
						unset( $plugins_to_install[ $key ] );
						continue;
					}

					// For install: make sure this is a plugin we *can* install and not one already installed.
					if ( 'install' === $install_type && true === $this->wpelemento_importer->is_plugin_installed( $slug ) ) {
						unset( $plugins_to_install[ $key ] );
					}

					// For updates: make sure this is a plugin we *can* update (update available and WP version ok).
					if ( 'update' === $install_type && false === $this->wpelemento_importer->is_plugin_updatetable( $slug ) ) {
						unset( $plugins_to_install[ $key ] );
					}
				}
				...
				foreach ( $plugins_to_install as $slug ) {
					$name   = $this->wpelemento_importer->plugins[ $slug ]['name'];
					$source = $this->wpelemento_importer->get_download_url( $slug );
					...
					if ( ! empty( $name ) && ! empty( $source ) ) {
						...
						switch ( $install_type ) {
							...
							case 'update':
								$file_paths[]                 = $this->wpelemento_importer->plugins[ $slug ]['file_path'];
								$to_inject[ $slug ]           = $this->wpelemento_importer->plugins[ $slug ];
								$to_inject[ $slug ]['source'] = $source;
								break;
						}
					}
				...
				// Create a new instance of WPELEMENTO_IMPORTER_TGMPA_Bulk_Installer.
				...
				add_filter( 'upgrader_source_selection', array( $this->wpelemento_importer, 'maybe_adjust_source_dir' ), 1, 3 );

				if ( 'wpelemento-importer-tgmpa-bulk-update' === $this->current_action() ) {
					// Inject our info into the update transient.
					$this->wpelemento_importer->inject_update_info( $to_inject );
				...
				remove_filter( 'upgrader_source_selection', array( $this->wpelemento_importer, 'maybe_adjust_source_dir' ), 1 );
			...
			if ( 'wpelemento-importer-tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( isset( $_POST['plugin'] ) ) {
					...
					$plugins = array_map( array( $this->wpelemento_importer, 'sanitize_key' ), $plugins );
				}
				...
				foreach ( $plugins as $slug ) {
					if ( $this->wpelemento_importer->can_plugin_activate( $slug ) ) {
						$plugins_to_activate[] = $this->wpelemento_importer->plugins[ $slug ]['file_path'];
						$plugin_names[]        = $this->wpelemento_importer->plugins[ $slug ]['name'];
					}
				}
				unset( $slug );
		 ...
		 *
		 * @since      2.2.0
		 * @deprecated 2.5.0 use {@see WPELEMENTO_IMPORTER_TGM_Plugin_Activation::_get_plugin_data_from_name()} instead.
		 * @see        WPELEMENTO_IMPORTER_TGM_Plugin_Activation::_get_plugin_data_from_name()
		 *
		 * @param string $name Name of the plugin, as it was registered.
		 * @param string $data Optional. Array key of plugin data to return. Default is slug.
		 * @return string|boolean Plugin slug if found, false otherwise.
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			_deprecated_function( __FUNCTION__, 'TGMPA 2.5.0', 'WPELEMENTO_IMPORTER_TGM_Plugin_Activation::_get_plugin_data_from_name()' );

			return $this->wpelemento_importer->_get_plugin_data_from_name( $name, $data );
		}
	}
}


if ( ! class_exists( 'WPELEMENTO_IMPORTER_TGM_Bulk_Installer' ) ) {
	 ...
	 * @since 2.5.2
	 *
	 * {@internal The WPELEMENTO_IMPORTER_TGMPA_Bulk_Installer class was originally called WPELEMENTO_IMPORTER_TGM_Bulk_Installer.
...
if ( ! function_exists( 'wpelemento_importer_tgmpa_load_bulk_installer' ) ) {
	...
	function wpelemento_importer_tgmpa_load_bulk_installer() {
		...
		$tgmpa_instance = call_user_func( array( get_class( $GLOBALS['wpelemento_importer_tgmpa'] ), 'get_instance' ) );
		...
		if ( isset( $_GET['page'] ) && $tgmpa_instance->menu === $_GET['page'] ) {
			...
			if ( ! class_exists( 'WPELEMENTO_IMPORTER_TGMPA_Bulk_Installer' ) ) {
				 ...
				 * {@internal Since 2.5.2 the class has been renamed from WPELEMENTO_IMPORTER_TGM_Bulk_Installer to WPELEMENTO_IMPORTER_TGMPA_Bulk_Installer.
				 ...
				 * @author  Gary Jones
				 */
				class WPELEMENTO_IMPORTER_TGMPA_Bulk_Installer extends Plugin_Upgrader {
					/**
					...
					public function __construct( $skin = null ) {
						...
						add_action( 'upgrader_process_complete', array( $this->wpelemento_importer, 'populate_file_path' ) );
					}
...
if ( ! class_exists( 'WPELEMENTO_IMPORTER_TGMPA_Utils' ) ) {
	 ...
	 */
	class WPELEMENTO_IMPORTER_TGMPA_Utils {
		/**
		 ...
		 * @return string
		 */
		public static function wpelemento_importer_wrap_in_em( $string ) {
		 ...
		 * @return string
		 */
		public static function wpelemento_importer_wrap_in_strong( $string ) {
		 ...
		 * @return bool
		 */
		public static function wpelemento_importer_validate_bool( $value ) {
			...
			if ( self::$has_filters ) {
				...
				return self::wpelemento_importer_emulate_filter_bool( $value );
			}
		}
